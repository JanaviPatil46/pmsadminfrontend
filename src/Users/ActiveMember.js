...
import { X, HelpCircle, Pencil, Trash2, UserMinus } from "lucide-react";
...
const ActiveMember = () => {
  ...
  const handleEdit = async (_id) => {
    setEditId(_id);
    const response = await fetch(`${LOGIN_API}/admin/teammember/` + _id);
    if (!response.ok) {
  ...
  const handleUpdateTeamMember = () => {
    ...
    const url = `${LOGIN_API}/admin/teammember/${editId}`;
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2.5">
            ...
            <Link to={linkPath} className="text-sm font-medium text-foreground hover:text-primary hover:underline transition-colors">
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => { handleEdit(member._id); handleUpdateDrawerOpen(); }}
            ...
            <button
              ...
              className="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:text-amber-600 hover:bg-amber-50"
              title="Deactivate"
            >
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable
        ...
        emptyDescription="Invite a team member to get started"
        pageSize={30}
      />
