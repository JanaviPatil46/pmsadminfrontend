...
console.log("hgjhg",templateId)
  ...
  useEffect(() => {
    if (decodedTemplateId) fetchFolderTree(decodedTemplateId);
  }, [decodedTemplateId]);

 // API call to fetch folder tree for a given template ID
  const fetchFolderTree = async (decodedTemplateId) => {
    try {
      const res = await fetch(`https://www.snptaxes.com/api/accountsdoc/files/list?folderPath=${decodedTemplateId}`);
  ...
  const updateStatus = async (item, statusType, newValue) => {
    try {
      ...
      if (res.ok) {
        ...
        fetchFolderTree(); // refresh folder tree to reflect change
      } else {
