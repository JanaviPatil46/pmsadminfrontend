...
//     try {
//       setLoading(true);
//       const response = await fetch(`http://localhost:9000/api/proposals/${proposalId}`);
//       if (!response.ok) {
...
//     productorService: item.productorService || '', // Match API field name
//     description: item.description || '',
//     rate: item.rate?.toString() || '0.00',
//     quantity: item.quantity?.toString() || '1', // Match API field name
...
//     // Find matching template to create the proper object
...
//   console.log("Itemized data from API:", itemizedData); // Debug log
...
//       teamMember: invoice.teamMember?.value || invoice.teamMember,
...
//         alert('Error submitting proposal: ' + errorText);
//       }
//     } catch (error) {
...
//           ← Back to Proposals
//         </button>
//         <h1>{proposalId ? `Edit Proposal: ${formData.general.proposalName}` : 'Create New Proposal'}</h1>
//       </div>
  ...
  const validateStep = (stepIndex) => {
    ...
    switch (stepKey) {
      ...
      case "services":
        ...
        // else if (formData.services.option === 'invoice') {
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Box>
        ...
        <Grid container spacing={3} mr={5} p={5}>
          ...
          >
            <Box
              sx={{ display: "flex", justifyContent: "space-between",flexDirection: "column",gap:2}}
            >
               <Button
