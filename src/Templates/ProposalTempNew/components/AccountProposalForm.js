...
const fetchProposalData = async () => {
  try {
    setLoading(true);
    setError('');
    const response = await fetch(`http://localhost:9000/account/proposals/${proposalId}`);
    if (!response.ok) {
    ...
    const [accountsResponse, templatesResponse] = await Promise.all([
      fetch(`${ACCOUNT_API}/accounts/account/accountdetailslist/true`),
      fetch('http://localhost:9000/api/proposals')
    ]);
    
    const accountsData = await accountsResponse.json();
    const templatesData = await templatesResponse.json();
    
    const accounts = accountsData.accountlist || accountsData || [];
...
// Validation functions for each step
 ...
 // Check if a step has errors for the stepper
  ...
  const handleBackToList = () => {
    navigate('/account-proposaltable');
  };
  ...
  const handleSubmit = async () => {
    ...
    // Prepare data for submission
      const submissionData = {
        ...
        general: {
        ...formData.general,
        account: accountId,           // ✅ Send only ID
      ...
      const url = proposalId 
        ? `http://localhost:9000/account/proposals/${proposalId}`
        : 'http://localhost:9000/account/proposals';
