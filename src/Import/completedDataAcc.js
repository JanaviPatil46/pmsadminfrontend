  ...
  const fetchAccounts = async () => {
    try {
      const res = await axios.get(
        "https://www.snptaxes.com/api/accounts/only-imported?active=true",
      );
...
// 👉 Delete all documents of an account
const handleDeleteDocuments = async (account) => {
  if (!account?._id) return;

  const confirmDelete = window.confirm(
    "Are you sure you want to delete ALL documents for this account?"
  );
  ...
  try {
    await axios.delete(
      "https://www.snptaxes.com/api/accountsdoc/delete-account-folder",
      {
                ...
                <TableCell align="right">
                  <Stack direction="row" spacing={1} justifyContent="flex-end">
                    ...
                    <Button
                      ...
                      disabled={
                        !selectedZip || selectedAccount?._id !== account._id
                      }
                    >
                      Upload
                    </Button>
